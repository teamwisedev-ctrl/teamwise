...
async function test() {
    try {
        ...
        const prodRes = await axios.post('https://wiseteam.cafe24api.com/api/v2/admin/products', productPayload, {
             headers: {
        ...
        const mapRes = await axios.post(`https://wiseteam.cafe24api.com/api/v2/admin/products/${productNo}/images`, mapPayload, {
             headers: {
                 'Authorization': `Bearer ${token}`,
                 'Content-Type': 'application/json'
             }
