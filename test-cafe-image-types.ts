...
async function test() {
    try {
        ...
        const prodRes = await axios.post('https://wiseteam.cafe24api.com/api/v2/admin/products', payloadA, {
             headers: {
    ...
    try {
        ...
        const prodRes = await axios.post('https://wiseteam.cafe24api.com/api/v2/admin/products', payloadC, {
             headers: {
                 'Authorization': `Bearer ${token}`,
                 'Content-Type': 'application/json'
             }
