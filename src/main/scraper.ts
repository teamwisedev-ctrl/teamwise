...
export async function scrapeDometopiaProduct(url: string, cookie?: string | null) {
  try {
    ...
    let rawPrice =
      ($('input[name="multi_discount_fifty"]').val() as string) ||
    ...
    const mainImageUrl = mainImageSrc.startsWith('http')
      ? mainImageSrc
      : mainImageSrc
        ? `https://dometopia.com${mainImageSrc}`
        : ''
    ...
    if ($spec.length) {
      ...
      $spec.find('img').each((_i, el) => {
        ...
        if (src && !src.startsWith('http')) {
          $(el).attr(
            'src',
            src.startsWith('/') ? `https://dometopia.com${src}` : `https://dometopia.com/${src}`
          )
        }
      })
      ...
      $spec.find('iframe').each((_i, el) => {
        ...
        if (src && (src.includes('youtube.com') || src.includes('youtu.be'))) {
          $(el).replaceWith(
            `<p><a href="${src}" target="_blank" style="font-size:16px; font-weight:bold; color:blue;">▶ 상품 소개 영상 보기 (클릭)</a></p>`
          )
        } else {
          $(el).remove()
        }
      ...
      $spec.find('img').each((_i, el) => {
        ...
        if (src.includes('all_top_img') || src.includes('gtd_title') || src.includes('top_ban')) {
          ...
          $(el).attr(
            'style',
            'max-width: 100%; height: auto; display: block; margin: 20px auto; border-radius: 8px;'
          )
        }
      })
      ...
      $spec.find('table').each((_i, el) => {
        ...
        $(el).attr(
          'style',
          'width: 100%; min-width: 300px; max-width: 100%; border-collapse: collapse; font-size: 14px; color: #444; text-align: left; border-top: 2px solid #333;'
        )
      })
      $spec.find('th, td').each((_i, el) => {
        $(el).attr(
          'style',
          'border-bottom: 1px solid #eee; padding: 12px 15px; line-height: 1.6; word-break: keep-all; vertical-align: top;'
        )
      })
      $spec.find('th').each((_i, el) => {
        ...
        $(el).attr(
          'style',
          currentStyle + ' background-color: #f9f9fa; font-weight: bold; width: 30%; color: #222;'
        )
      })
      $spec.find('table').each((_i, el) => {
      ...
      $spec.find('p, div').each((_i, el) => {
        ...
        if ($(el).find('img').length === 0 && $(el).text().trim() !== '') {
          $(el).attr(
            'style',
            'line-height: 1.8; color: #222; word-break: keep-all; font-size: 16px; margin: 15px auto; text-align: center; max-width: 800px; padding: 0 15px;'
          )
        }
      })
      ...
      detailImages.each((_i, el) => {
        ...
        if (src && !src.includes('all_top_img')) {
          ...
          if (!canonicalSrc.startsWith('http')) {
            canonicalSrc = canonicalSrc.startsWith('/')
              ? `https://dometopia.com${canonicalSrc}`
              : `https://dometopia.com/${canonicalSrc}`
          }
    ...
    if (!detailHtml || detailHtml.trim() === '') {
      detailHtml =
        '<p style="text-align: center; color: #666; font-size: 14px; padding: 50px 0;">상세설명 참조</p>'
    }
    ...
    $spec.find('p, div, span').each((_i, el) => {
      ...
      const kcMatch = text.match(
        /([A-Z]{2,3}[0-9]{4,5}[A-Z]?[0-9]*-[0-9A-Za-z]+|R-[A-Z]-[a-zA-Z0-9]+-[a-zA-Z0-9]+|CB[0-9]{2}[A-Za-z][0-9]+-[0-9]+|SU[0-9]{2}[A-Za-z][0-9]+-[0-9]+)/
      )
      if (kcMatch) {
    ...
    if (!kcCertification) {
      const htmlText = $.html() || ''
      const htmMatch = htmlText.match(
        /([A-Z]{2,3}[0-9]{4,5}[A-Z]?[0-9]*-[0-9A-Za-z]+|R-[A-Z]-[a-zA-Z0-9]+-[a-zA-Z0-9]+)/
      )
      if (htmMatch) {
 ...
 */
export async function scrapeCategoryLinks(
  baseUrl: string,
  cookie?: string | null
 ...
 */
export async function checkDometopiaStatus(
  itemCode: string,
  cookie?: string | null
): Promise<{ isOutOfStock: boolean; currentPrice?: number; error?: string }> {
  try {
    ...
    if (
      ...
      if (
        html.includes('alert(') &&
        (html.includes('history.back') || html.includes('location.href'))
      ) {
        return { isOutOfStock: true }
      }
    }
