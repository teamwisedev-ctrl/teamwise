...
export async function createCafe24Product(mallId: string, payload: Cafe24ProductPayload) {
...
export async function deleteCafe24Product(mallId: string, productNo: number) {
  const token = await getCafe24Token(mallId)

  try {
    await axios.delete(`https://${mallId}.cafe24api.com/api/v2/admin/products/${productNo}`, {
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
...
export async function fetchCafe24Orders(mallId: string, startDate: string, endDate: string) {
  ...
  try {
    const response = await axios.get(`https://${mallId}.cafe24api.com/api/v2/admin/orders`, {
      params: {
    ...
    const formattedOrders = (response.data.orders || []).map((o: any) => {
      ...
      const addressStr =
        ...
        `${receiver.address1 || ''} ${receiver.address2 || ''}`.trim() ||
        `${buyer.buyer_address1 || ''} ${buyer.buyer_address2 || ''}`.trim()
      const amountStr =
        o.initial_order_amount?.total_amount_due || o.actual_order_amount?.total_amount_due || '0'
...
export async function fetchCafe24Categories(mallId: string) {
  ...
  try {
    const response = await axios.get(`https://${mallId}.cafe24api.com/api/v2/admin/categories`, {
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
...
export async function createCafe24Category(
  mallId: string,
  categoryName: string,
  _isRetry = false
): Promise<any> {
  ...
  try {
    ...
    if (
      !_isRetry &&
      (status === 401 || status === 403 || errorMessage.includes('Insufficient_scope'))
    ) {
