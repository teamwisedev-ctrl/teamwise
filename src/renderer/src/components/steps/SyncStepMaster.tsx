...
export const SyncStepMaster: React.FC<SyncStepProps> = ({
  masterSheetId,
  activePlans = [],
  licenseTier,
  betaMarketsInfo = {},
    ...
    try {
      ...
      const readRes = await Promise.race([
        ...
        new Promise<any>((_, reject) =>
          setTimeout(
            () => reject(new Error('마스터 DB 로딩 시간 초과 (15초). 앱을 재시작해주세요.')),
            15000
          )
        )
      ])
      ...
      const masterProducts = rows
        ...
        .filter(
          (p) =>
            p.channelProductNo &&
            !p.itemCode.includes('[스토어삭제됨]') &&
            !p.vendor.includes('[스토어삭제됨]')
        )
      ...
      addLog(`\n🎉 씽크 작업이 모두 완료되었습니다!`)
      addLog(
        `결과: 상태복원 ${successCount}건, 유지 ${skipCount}건, DB청소/스토어삭제 ${deletedCount}건, 실패 ${errorCount}건`
      )
    } catch (err: unknown) {
      const errMsg = err instanceof Error ? err.message : String(err)
  ...
  const handleMonitorSync = async () => {
    ...
    try {
      addLog('1. 마스터 DB 전체 상품 목록 스캔 중...')
      const readRes = await window.electron.ipcRenderer.invoke(
        'read-master-sheet-full',
        masterSheetId
      )
      ...
      if (total > 500 && !activePlans.includes('pro_unlimited')) {
        addLog(
          `⚠️ 스타터 요금제(무료)는 1회 최대 500건까지만 조회 및 동기화가 가능합니다. 현재 ${total}건 감지됨.`
        )
        addLog('⚠️ 제한 없는 고속 1:N 씽크를 원하시면 [슈퍼셀러 무제한팩]을 구독해주세요!')
        if (
          window.confirm(
      ...
      for (let i = 0; i < total; i++) {
        ...
        try {
          ...
          const dometopiaRes = await window.electron.ipcRenderer.invoke(
            'check-dometopia-status',
            product.itemCode
          )
          ...
          // 3. 가격 감시 및 업데이트
            ...
            const newSalePrice =
              Math.floor(
                (dometopiaRes.currentPrice * (1 + marginRate / 100) + extraShippingCost) / 10
              ) * 10
            ...
            addLog(
              `  💰 도매가 정상 확인됨 (${dometopiaRes.currentPrice.toLocaleString()}원). 스마트스토어 갱신(${newSalePrice.toLocaleString()}원) 진행...`
            )
      ...
      addLog(
        `결과: 가격 갱신/유지 ${successCount}건, 품절처리 ${outOfStockCount}건, 오류 ${errorCount}건`
      )
    } catch (err: unknown) {
      const errMsg = err instanceof Error ? err.message : String(err)
      addLog(`❌ 동기화 중 오류가 발생했습니다: ${errMsg}`)
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
        <button
          ...
          style={{
            ...
            padding: '12px',
            background:
              activeTab === 'statusSync' ? 'var(--color-primary)' : 'var(--color-surface-elevated)',
            color: activeTab === 'statusSync' ? '#fff' : 'var(--color-text)',
            border: '1px solid',
            borderColor:
              activeTab === 'statusSync' ? 'var(--color-primary)' : 'var(--color-border)',
            borderRadius: '8px',
            fontWeight: 600,
            cursor: 'pointer',
            transition: 'all 0.2s'
        ...
        <button
          ...
          style={{
            ...
            padding: '12px',
            background:
              activeTab === 'monitorSync'
                ? 'rgba(16, 185, 129, 0.2)'
                : 'var(--color-surface-elevated)',
            color: activeTab === 'monitorSync' ? '#34d399' : 'var(--color-text)',
            border: '1px solid',
            borderColor:
              activeTab === 'monitorSync' ? 'rgba(16, 185, 129, 0.4)' : 'var(--color-border)',
            borderRadius: '8px',
            fontWeight: 600,
            cursor: 'pointer',
            transition: 'all 0.2s'
      ...
      <div className="glass-panel" style={{ marginBottom: '24px' }}>
        {activeTab === 'statusSync' ? (
          ...
          <>
            <div className="panel-title" style={{ color: '#34d399' }}>
              <ShieldCheck size={20} style={{ marginRight: '8px' }} /> 도매토피아 자동 가격/단종
              모니터링
            </div>
            <p
              style={{
            ...
            >
              마스터 DB에 등록된 도매토피아 상품들의 <b>현재 도매상태(가격 변동, 단종/품절)</b>를
              빠르게 조회하여, 스마트스토어의 판매가격과 판매상태를 실시간으로 업데이트합니다.
            </p>
          </>
        )}
        ...
        {activeTab === 'statusSync' ? (
          ...
          >
            ...
            >
              <li>
                마스터 DB에 존재하지 않는 상품(판매자가 수동 등록한 자체 상품 등)은 스캔을 회피하여
                안전하게 보호됩니다.
              </li>
              <li>
                스토어에서 삭제된 데이터(404 Not Found) 발견 시 마스터 DB를 청소하여 쓰레기 데이터를
                비웁니다.
              </li>
              <li>
                마스터 DB에서 상품명에 [단종] 등 특수 표기가 들어간 행은 스토어에서 판매중지/삭제
                처리하여 동기화합니다.
              </li>
            </ul>
          </div>
          ...
          >
            ...
            >
              <Coins size={16} /> 일괄 적용할 가격 산정 공식 (마진율 + 마진)
            </div>
            ...
            <p style={{ marginTop: '12px', fontSize: '13px', color: '#cbd5e1', marginBottom: 0 }}>
              * 도매가가 인상/인하 되었을 경우, 위 공식 <b>(변경된 도매가 * 마진율 + 고정수익)</b>
              으로 판매가를 자동으로 덮어씁니다.
            </p>
          </div>
        )}
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
            {[
              ...
              {
                id: 'coupang',
                label: activePlans.includes('addon_coupang') ? '쿠팡' : '🔒 쿠팡 (Add-on 필요)',
                color: '#cb1400',
                requirePro: true
              },
              {
              ...
              if (isLockedFreePlan && market.id !== 'coupang')
                labelContent = `🔒 ${market.label} (Pro 전용)`
              if (isBetaTarget && licenseTier === 'free')
                labelContent = `${market.label} (BETA 무료)`
              ...
              return (
                <label
                  ...
                  style={{
                    ...
                    padding: '8px 12px',
                    borderRadius: '6px',
                    border: targetMarkets[market.id]
                      ? `1px solid ${market.color}`
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (market.id === 'coupang' && !activePlans.includes('addon_coupang')) {
                        if (
                          window.confirm(
                            '쿠팡 연동은 [쿠팡 전용 확장팩] 구매가 필요한 기능입니다. 결제 페이지로 이동하시겠습니까?'
                          )
                        ) {
                          window.electron.ipcRenderer.send(
                            'open-external-window',
                            'https://mo2.kr/pricing'
                          )
                        }
                        return
                      }
        ...
        >
          {isSyncing ? (
            <>
              ...
              />
              <span>
                작업 진행 중... ({syncProgress}/{syncTotal})
              </span>
            </>
          ) : (
            <>
