...
export const DataPrepStep: React.FC<DataPrepStepProps> = ({
  ...
  const getPlaceholder = () => {
    ...
    if (scrapeMethod === 'product')
      return '도매토피아 상품번호 입력 (여러 개일 경우 줄바꿈으로 구분)'
      ...
      <div className="glass-panel" style={{ marginBottom: '24px' }}>
        ...
        >
          <div className="panel-title" style={{ margin: 0 }}>
            <Database size={20} color="#3b82f6" /> 데이터베이스 연동
          </div>
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <div style={{ fontWeight: 600, marginBottom: '4px' }}>
                  스프레드시트가 성공적으로 연동되었습니다
                </div>
                <div style={{ opacity: 0.8, fontSize: '13px', fontFamily: 'monospace' }}>
                  ID: {sheetId}
                </div>
              </div>
            </div>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {[
              ...
              { id: 'domejjim', label: '도매찜', ready: false }
            ].map((s) => (
              <button
                key={s.id}
                onClick={() => {
                ...
                style={{
                  ...
                  transition: 'all 0.2s',
                  boxShadow:
                    wholesaleSource === s.id ? '0 2px 4px rgba(59, 130, 246, 0.1)' : 'none',
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
          <button
            ...
            style={{
              ...
              padding: '16px',
              background:
                activeTab === 'manual' ? 'var(--color-primary)' : 'var(--color-surface-elevated)',
          ...
          <button
            ...
            style={{
              ...
              padding: '16px',
              background:
                activeTab === 'auto' ? 'var(--color-primary)' : 'var(--color-surface-elevated)',
        ...
        <div className="glass-panel" style={{ minHeight: '300px' }}>
          {activeTab === 'manual' && (
            <div className="animate-fade-in" style={{ textAlign: 'center', padding: '40px 20px' }}>
              ...
              >
                상단의 <strong>[✨ 브라우저에서 열기]</strong> 버튼을 눌러 생성된 구글 시트에 접속한
                뒤, 규격화된 헤더 양식에 맞추어 원하는 판매 상품의 정보를 복사/붙여넣기 하세요.
                <br />
                <br />
                <span style={{ color: 'var(--color-primary)', fontWeight: 500 }}>
                  입력을 완료했다면 우측 하단의 [다음 단계]를 눌러 스토어로 전송하세요.
                </span>
              </p>
          ...
          {activeTab === 'auto' && (
            <div className="animate-fade-in">
              <h3
                style={{ fontSize: '18px', fontWeight: 600, marginBottom: '8px', color: '#f8fafc' }}
              >
                🔍 도매토피아 연동 자동화
              </h3>
              <p
              ...
              >
                도매처의 상품 데이터(상품명, 가격, 이미지, 상세설명)를 즉시 스크래핑하여 엑셀 양식에
                맞춰 자동으로 채워 넣습니다.
              </p>
              ...
              <div
                style={{
                  marginBottom: '24px',
                  padding: '16px 20px',
                  backgroundColor: dometopiaSession
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    <div
                      style={{
                        fontWeight: 600,
                        color: dometopiaSession ? '#34d399' : '#f8fafc',
                        marginBottom: '4px'
                      }}
                    >
                      {dometopiaSession
                    ...
                    <div style={{ fontSize: '13px', color: '#94a3b8' }}>
                      {dometopiaSession
                        ? '실제 도매 회원가 및 할인율이 반영되어 정확하게 수집됩니다.'
                        : '로그인하지 않으면 비정상적인 가격(소비자가)이 연동될 수 있습니다.'}
                    </div>
                  </div>
                </div>
                <button
              ...
              >
                ...
                >
                  <input
                    type="radio"
                    value="product"
                    checked={scrapeMethod === 'product'}
                    onChange={() => setScrapeMethod('product')}
                ...
                >
                  <input
                    type="radio"
                    value="category"
                    checked={scrapeMethod === 'category'}
                    onChange={() => setScrapeMethod('category')}
                    style={{ accentColor: 'var(--color-primary)', width: '18px', height: '18px' }}
                  />
                  📂 카테고리 (대량)
                </label>
                ...
                >
                  <input
                    type="radio"
                    value="search"
                    checked={scrapeMethod === 'search'}
                    onChange={() => setScrapeMethod('search')}
                    style={{ accentColor: 'var(--color-primary)', width: '18px', height: '18px' }}
                  />
                  🔍 검색어 (대량)
                </label>
