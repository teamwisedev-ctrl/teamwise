...
export const MarketSyncStep: React.FC<MarketSyncStepProps> = ({
  ...
  React.useEffect(() => {
    if (
      selectedMarkets.includes('cafe24') &&
      ...
      window.electron.ipcRenderer
        .invoke('get-cafe24-categories', { mallId: cafe24Credentials.mallId })
        .then((res) => {
          if (isTimeout) return
  ...
  const handleInitiateSync = () => {
    ...
    if (selectedMarkets.includes('cafe24') && !cafe24Credentials.mallId) {
      alert(
        '카페24 쇼핑몰 아이디(Mall ID) 설정이 필요합니다.\n[환경 설정] 탭에서 우선 연동해주세요.'
      )
      return
    }
    ...
    if (
      selectedMarkets.includes('smartstore') &&
      (!credentials.clientId || !credentials.clientSecret)
    ) {
      alert(
        '스마트스토어 API 연동을 위한 Client ID와 Client Secret 설정이 필요합니다.\n[환경 설정] 탭에서 우선 연동해주세요.'
      )
      return
    }
    ...
    <div className="animate-fade-in">
      <div className="glass-panel" style={{ marginBottom: '32px' }}>
        <div className="panel-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <RefreshCcw size={20} color="#3b82f6" /> 상품 연동 허브
        </div>
        <p style={{ color: '#cbd5e1', marginBottom: '24px', fontSize: '15px', lineHeight: '1.6' }}>
          구글 시트에서 미리 세팅된 데이터를 읽어오고 수익률을 지정한 후, 다중 마켓에 일괄
          등록합니다.
        </p>
      ...
      {sheetData.length > 0 && (
        <>
          <div className="glass-panel" style={{ marginBottom: '32px' }}>
            ...
            >
              <Coins size={20} color="#34d399" /> 2. 수익률 설정 및 데이터 검토
            </div>
            <p
              style={{
                color: '#cbd5e1',
              ...
              <table className="data-table">
                ...
                <tbody>
                  {(() => {
                    const hasHeader =
                      sheetData.length > 0 &&
                      (sheetData[0][0] === '카테고리ID' || sheetData[0][1] === '상품명')
                          ...
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              <span style={{ fontSize: '15px', fontWeight: 600, color: '#38bdf8' }}>
                                {(
                                  Math.floor(
                                    (parseInt(row[4] || '0') * (1 + marginRate / 100) +
                                      extraShippingCost) /
                                      10
                                  ) * 10
                                ).toLocaleString()}{' '}
          ...
          <div
            ...
            style={{ marginBottom: '32px', textAlign: 'left', padding: '32px' }}
          >
            <h4
              style={{
                margin: '0 0 16px 0',
              ...
              {targetMarkets.map((market) => {
                ...
                return (
                    ...
                    style={{
                      ...
                      border: isLocked
                        ...
                        : selectedMarkets.includes(market.id)
                          ...
                          : '1px solid #e2e8f0',
                      borderRadius: '12px',
                      cursor: market.ready && !isLocked ? 'pointer' : 'not-allowed',
                      ...
                      color: isLocked
                        ...
                        : selectedMarkets.includes(market.id)
                          ...
                          : '#64748b',
                      transition: 'all 0.2s',
                      boxShadow: selectedMarkets.includes(market.id)
                  ...
                  >
                    ...
                    {!market.ready && !isBetaTarget && !isLocked && (
                      <span
                        style={{
                          fontSize: '12px',
                          color: '#94a3b8',
            ...
            {selectedMarkets.includes('cafe24') && cafe24Credentials.connected && (
              ...
              >
                ...
                >
                  <FolderOpen size={18} color="#f59e0b" /> 카페24 연동 시, 어떤 카테고리로 상품을
                  넣을까요?
                </label>
                ...
                {isLoadingCategories ? (
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    ...
                    >
                      <XCircle size={16} /> {categoryLoadError} (임시 보관함을 자동 생성하여
                      진행해주세요.)
                    </div>
                    <button
                      className="secondary"
                ...
                >
                  선택한 한 곳의 카테고리로 데이터가 일괄 전송됩니다. 카페24 전용 임시 보관함을
                  만들어두고 나중에 카페24 어드민에서 상세 분류하는 것을 권장합니다.
                </p>
              </div>
            )}
          </div>
          ...
          <div
            className="glass-panel"
            style={{ marginBottom: '32px', textAlign: 'center', padding: '32px' }}
          >
            <div
              style={{
                fontSize: '32px',
            ...
            <h3
              style={{ margin: '0 0 16px 0', fontSize: '22px', color: '#1e293b', fontWeight: 700 }}
            >
              모든 준비가 완료되었습니다!
            </h3>
            <p style={{ color: '#64748b', marginBottom: '24px', fontSize: '15px' }}>
              위의 상품 목록과 최종 가격, 그리고 타겟 마켓({selectedMarkets.length}곳) 설정을 확인한
              후 실행하세요.
            </p>
            <button
              className="primary"
              onClick={handleInitiateSync}
              disabled={sheetData.length === 0 || selectedMarkets.length === 0}
              style={{
                width: '100%',
                padding: '16px',
                fontSize: '16px',
                fontWeight: 600,
                opacity: sheetData.length === 0 || selectedMarkets.length === 0 ? 0.5 : 1,
                cursor:
                  sheetData.length === 0 || selectedMarkets.length === 0
                    ? 'not-allowed'
                    : 'pointer',
                transition: 'all 0.2s',
                display: 'flex',
            ...
            >
              <CloudUpload size={20} /> 선택된 {selectedMarkets.length}개 마켓으로 일괄 배포
              시작하기
            </button>
          </div>
        </>
