    ...
    <div className="animate-fade-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div className="glass-panel" style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: '#cbd5e1', marginBottom: '24px', fontSize: '15px', lineHeight: '1.6' }}>
          앱 구동 및 로그인 시 자동으로 사용자 구글 드라이브에 시트가 생성되며, 상품 대량 연동 시{' '}
          <strong>카테고리 매핑 인메모리 캐시</strong>로 활용됩니다.
        </p>
      ...
      <div className="glass-panel" style={{ marginBottom: '32px' }}>
        <div className="panel-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Settings size={20} color="#3b82f6" /> 다중 마켓 플랫폼 인증 설정
        </div>
        <p style={{ color: '#cbd5e1', marginBottom: '32px', fontSize: '15px', lineHeight: '1.6' }}>
          상품과 주문을 연동할 외부 쇼핑몰 플랫폼의 접근 권한(API Key)을 연동합니다. 한 번 저장된
          정보는 안전하게 브라우저 로컬 환경에 보관됩니다.
        </p>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          >
            <p style={{ fontSize: '14px', color: '#94a3b8', marginBottom: '16px' }}>
              복잡한 키 발급 없이 쇼핑몰 ID만 입력하고 <strong>[연동하기]</strong> 버튼을 누르면
              즉시 관리 권한이 부여됩니다.
            </p>
            <div
              style={{
            ...
            <button
              ...
              style={{
                ...
                gap: '8px',
                fontSize: '15px',
                opacity: isCafe24Authenticating ? 0.7 : 1
              }}
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  await fetch(`https://${cafe24Credentials.mallId}.cafe24api.com/`, {
                    mode: 'no-cors'
                  })
                } catch {
                  alert(
                ...
                addLog(
                  `카페24(${cafe24Credentials.mallId}) 연동 브라우저 창을 띄웁니다. 새 창에서 권한 동의를 완료해 주세요.`
                )
                ...
                const pollInterval = setInterval(async () => {
                  ...
                  try {
                    const res = await fetch(
                      `https://teamwise-sand.vercel.app/api/market/cafe24/token`,
                      {
                        method: 'POST',
                    ...
                    if (data && data.access_token) {
                      ...
                      setIsCafe24Authenticating(false)
                      addLog(
                        `✅ 카페24(${cafe24Credentials.mallId}) 권한 연동 성공! 웹훅 및 API가 정상적으로 연결되었습니다.`
                      )
                    }
                  } catch {
                    // Ignore
                  ...
                  if (attempts > 120) {
                    ...
                    setIsCafe24Authenticating(false)
                    addLog(
                      `❌ 카페24 인증 시간이 초과되었습니다. 창이 닫혔거나 통신 오류가 발생했습니다.`
                    )
                  }
                }, 1500)
              }}
        ...
        {licenseTier !== 'free' && (
          <div style={{ marginBottom: '16px' }}>
            ...
            >
              <p style={{ fontSize: '14px', color: '#94a3b8', marginBottom: '16px' }}>
                커머스 API 센터에서 발급받은 '애플리케이션 ID'와 '시크릿'을 입력하세요.
              </p>
