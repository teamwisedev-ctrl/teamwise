  ...
  try {
    ...
    if (!accessToken) {
      return NextResponse.json(
        { success: false, error: 'Google 인증 토큰이 제공되지 않았습니다.' },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: `구글 인증 에러: ${verifyError.message}` },
        { status: 401 }
      )
    }
    ...
    if (profileError || !profile) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `[${email}] 웹사이트 가입 이력이 없습니다. 먼저 웹사이트(Mo2)에 가입하고 요금제를 선택해 주세요.`
        },
        { status: 403 }
      )
    }
    ...
    const isValidStatus =
      subData &&
      (subData.status === 'active' || subData.status === 'trialing' || subData.status === 'trial')
    const hasProPlan =
      isValidStatus &&
      (subData.tier === 'pro' ||
        subData?.plan_id?.includes('pro') ||
        subData?.plan_id?.includes('premium'))
