  ...
  try {
    ...
    if (event_type === 'app_uninstalled' && mall_id) {
      ...
      if (expectedClientId && client_id !== expectedClientId) {
        console.warn(
          `[Cafe24 Webhook] Client ID mismatch. Expected ${expectedClientId}, got ${client_id}`
        )
        return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
      }

      // Clean up the integration tokens to revoke access and free up the user's slot
      const { error: deleteError } = await adminClient.from('market_credentials').delete().match({
        market_type: 'cafe24',
        mall_id: mall_id
      })
      ...
      if (deleteError) {
        console.error(
          `[Cafe24 Webhook] Failed to delete credentials for mall ${mall_id}:`,
          deleteError.message
        )
      ...
      return NextResponse.json(
        { success: true, message: 'App uninstalled handling complete.' },
        { status: 200 }
      )
    }
