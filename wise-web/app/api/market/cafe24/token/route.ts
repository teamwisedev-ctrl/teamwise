  ...
  try {
    ...
    if (dbError || !credentials || !credentials.access_token) {
      ...
      return NextResponse.json(
        { error: 'OAuth credentials not found. Please connect Cafe24 again.' },
    ...
    if (expiresAt && new Date(now).getTime() > new Date(expiresAt).getTime() - 300000) {
      ...
      if (!clientId || !clientSecret || !credentials.refresh_token) {
        return NextResponse.json(
          { error: 'Cannot refresh token due to missing client credentials or refresh token.' },
      ...
      if (!refreshResponse.ok) {
        ...
        return NextResponse.json(
          ...
          { status: refreshResponse.status, headers: corsHeaders }
        )
      }
      ...
      return NextResponse.json(
        { success: true, access_token: newAccessToken, source: 'refreshed' },
        { headers: corsHeaders }
      )
    }
    ...
    return NextResponse.json(
      { success: true, access_token: credentials.access_token, source: 'cache' },
      { headers: corsHeaders }
    )
  } catch (error: any) {
