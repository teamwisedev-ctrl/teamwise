...
export default async function AdminPage() {
  ...
  const statusStr =
    rawStatusStr === 'trial' || rawStatusStr === 'trialing' ? 'active' : rawStatusStr
  const statusText =
    statusStr === 'active' ? 'Active' : statusStr.charAt(0).toUpperCase() + statusStr.slice(1)
  const statusColor = hasActiveSub
    ? '#10B981'
    : statusStr === 'none'
      ? 'var(--text-muted)'
      : '#EF4444'
  const statusBg = hasActiveSub
    ? 'rgba(16, 185, 129, 0.2)'
    : statusStr === 'none'
      ...
      <div style={statsGridStyles}>
        <div className="glass-panel animate-slide-up delay-100" style={statCardStyles}>
          ...
          <div style={statValueStyles}>
            {hasActiveSub ? 1 : 0}{' '}
            <span style={{ fontSize: '1rem', fontWeight: 'normal' }}>개</span>
          </div>
        </div>
        <div className="glass-panel animate-slide-up delay-100" style={statCardStyles}>
          ...
          <div
            style={{
              ...statValueStyles,
              color: hasActiveSub ? 'var(--accent-secondary)' : 'var(--text-muted)',
              fontSize: '1.5rem'
            }}
          >
            {planName}
            {hasActiveSub && (
              <span
                style={{ fontSize: '1rem', fontWeight: 'normal', color: 'var(--text-primary)' }}
              >
                {' '}
                (이용 중)
              </span>
