...
async function testFetchOrders() {
  ...
  console.log(
    `Fetching Cafe24 orders for ${mallId} from ${formatDate(lastWeek)} to ${formatDate(today)}...`
  )
  ...
  try {
    ...
    if (res.success && res.orders) {
      ...
      if (res.orders.length > 0) {
        ...
        console.log(
          'No orders found in this period. The integration is working, just no test data.'
        )
      }
    } else {
